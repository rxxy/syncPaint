...
historyCanvas.push({data:cxt.getImageData(0,0,canvasWidth,canvasHeight),x:0,y:0});
//存放当前画布数据
var currentImg;
var token;
...
socket.on('makecode', function(data){	//监听得到token
	token = data;
	$('#qrcode').qrcode({text:"http://"+window.location.host+"/client?token="+token});
});
...
socket.on('drawEnd', function(point){
	...
	historyCanvas[++current] = ({data:cxt.getImageData(0,0,canvasWidth,canvasHeight),x:p_left,y:p_top});
...
function positionChange(){
  ...
  // cxt.drawImage(img,p_left, p_top,rectWidth,rectHeight,0,0,rectWidth,rectHeight);
	...
	socket.emit('positionChange',{token:token,imgData:{data:imgData}});
}
$("#save").click(function(){
